...
logger_error_modifiedsemantic = logging.getLogger(
    'error_last-modified-semantic-change')
...
logger_info_zede_by_url = logging.getLogger(
    'info_productid_zeit-online-per-url')
...
logger_info_lastsem = logging.getLogger(
    'info_date-first-released_attr-last-semantic-change')
logger_info_rele = logging.getLogger('info_date-first-released_attr-release')
logger_info_year_vol = logging.getLogger(
    'info_date-first-released_year-volume')
logger_info_just_year = logging.getLogger('info_date-first-released_just-year')
logger_info_year_vol_copyr = logging.getLogger(
    'info_date-first-released_year-volume-copyrights')
logger_info_date_copyr = logging.getLogger(
    'info_date-first-released_date-copyrights')
...
logger_lps_last_semantic_change = logging.getLogger(
    'date_last_published_semantic-by-last_semantic_change')
logger_lps_date_last_modified = logging.getLogger(
    'date_last_published_semantic-by-date_last_modified')
logger_lps_date_first_released = logging.getLogger(
    'date_last_published_semantic-by-date_first_released')
logger_lps_date_first_release = logging.getLogger(
    'date_last_published_semantic-by-date_first_release')

logger_lp_date_last_modified = logging.getLogger(
    'date_last_published-by-date_last_modified')
logger_lp_last_semantic_change = logging.getLogger(
    'date_last_published-by-last_semantic_change')
logger_lp_date_first_released = logging.getLogger(
    'date_last_published-by-date_first_released')
logger_lp_date_first_release = logging.getLogger(
    'date_last_published-by-date_first_release')

logger_lm_last_semantic_change = logging.getLogger(
    'date_last_modified-by-last_semantic_change')
logger_lm_date_first_released = logging.getLogger(
    'date_last_modified-by-date_first_released')
logger_lm_date_first_release = logging.getLogger(
    'date_last_modified-by-date_first_release')
...
class XmlWorker(object):
    ...
    def get_ressorts(self):
        tree = etree.parse(
            os.path.dirname(__file__) + '/xmlworker/navigation.xml')
        return tree
    ...
    def make_checkable(self, uri, connector, logger):
        try:
            ...
            urgent = zeit.care.worker.get_property(
                connector[uri],
                'urgent', 'http://namespaces.zeit.de/CMS/workflow')
            edited = zeit.care.worker.get_property(
                connector[uri],
                'edited', 'http://namespaces.zeit.de/CMS/workflow')
    ...
    def create_year(self, uri, tree, logger):
        try:
            ...
            if tree.xpath('//attribute[@name="copyrights"]'):
                attr_copyrights = tree.xpath(
                    '//attribute[@name="copyrights"]')[0]
            ...
            if tree.xpath('//attribute[@name="last-semantic-change"]'):
                attr_last_semantic_change = tree.xpath(
                    '//attribute[@name="last-semantic-change"]')[0]
                last_semantic_change = attr_last_semantic_change.text
                match = re.search(
                    r"[1-2][0,9][0-9][0-9]", last_semantic_change)
                if match:
                    year = match.group(0)
                    return year
            if tree.xpath('//attribute[@name="date_last_published"]'):
                attr_date_last_published = tree.xpath(
                    '//attribute[@name="date_last_published"]')[0]
            ...
            if tree.xpath('//attribute[@name="date-last-modified"]'):
                attr_date_last_modified = tree.xpath(
    ...
    def get_properties(self, uri, tree, ressorts, connector, logger):
        try:
            uri = unicode(uri.decode("utf8"))
            type = zeit.care.worker.get_property(
                connector[uri], 'type', 'http://namespaces.zeit.de/CMS/meta')
            dailynl = zeit.care.worker.get_property(
                connector[uri],
                'DailyNL', 'http://namespaces.zeit.de/CMS/document')
            artbox_thema = zeit.care.worker.get_property(
                connector[uri],
                'artbox_thema', 'http://namespaces.zeit.de/CMS/document')
            banner = zeit.care.worker.get_property(
                connector[uri],
                'banner', 'http://namespaces.zeit.de/CMS/document')
            breaking_news = zeit.care.worker.get_property(
                connector[uri],
                'breaking_news', 'http://namespaces.zeit.de/CMS/document')
            comments = zeit.care.worker.get_property(
                connector[uri],
                'comments', 'http://namespaces.zeit.de/CMS/document')
            countings = zeit.care.worker.get_property(
                connector[uri],
                'countings', 'http://namespaces.zeit.de/CMS/document')
            foldable = zeit.care.worker.get_property(
                connector[uri],
                'foldable', 'http://namespaces.zeit.de/CMS/document')
            has_recensions = zeit.care.worker.get_property(
                connector[uri],
                'has_recensions', 'http://namespaces.zeit.de/CMS/document')
            in_rankings = zeit.care.worker.get_property(
                connector[uri],
                'in_rankings', 'http://namespaces.zeit.de/CMS/document')
            is_content = zeit.care.worker.get_property(
                connector[uri],
                'is_content', 'http://namespaces.zeit.de/CMS/document')
            minimal_header = zeit.care.worker.get_property(
                connector[uri],
                'minimal_header', 'http://namespaces.zeit.de/CMS/document')
            show_commentthread = zeit.care.worker.get_property(
                connector[uri],
                'show_commentthread', 'http://namespaces.zeit.de/CMS/document')
            ressort = zeit.care.worker.get_property(
                connector[uri],
                'ressort', 'http://namespaces.zeit.de/CMS/document')
            sub_ressort = zeit.care.worker.get_property(
                connector[uri],
                'sub_ressort', 'http://namespaces.zeit.de/CMS/document')
            print_ressort = zeit.care.worker.get_property(
                connector[uri],
                'ressort', 'http://namespaces.zeit.de/CMS/print')
            year = zeit.care.worker.get_property(
                connector[uri],
            ...
            if not has_recensions:
                properties.update({(
                    'has_recensions',
                    'http://namespaces.zeit.de/CMS/document'): "no"})
            ...
            if properties:
                properties.update({
                    ('published',
                     'http://namespaces.zeit.de/CMS/workflow'): 'yes'})
                crawler = zeit.care.crawl.ResourceProcess(
                    uri,
                    connector,
                    zeit.care.worker.property_worker,
    ...
    def _find_out_productid(self, connector, uri, tree, logger):
        ...
        try:
            match = re.search(
                r"xml.zeit.de/online/[1-2][0-9][0-9][0-9]/[0-9][0-9]", uri)
            if match:
            ...
            logger.error(
                "Exception _find_out_product_id " + uri + " " + str(e))
            return False
    ...
    def _create_date_first_released(self, connector, uri, tree, logger):
        try:
            ...
            if tree.xpath('//attribute[@name="date"]'):
                ...
                match = re.search(
                    r"[0-2][0-9].[0-1][0-9].[1-2][0-9][0-9][0-9]",
                    daterel.text)
                if match:
                        dstr = match.group(0)
                        dates = dstr.split(".")
                        logger_info_date_copyr.info(uri)
                        return "%s-%s-%sT12:00:00Z" % (
            ...
            if (tree.xpath('//attribute[@name="year"]') and
                ...
                match = re.search(
                    r"[0-9][0-9]/[1-2][0-9][0-9][0-9]", copyrights)
            ...
            if (year and year not in '' and
                ...
                if res is not False:
                    ...
                    while daterel is False:
                        if tree.xpath(
                                '/page/body/cluster[@area="feature"]'
                                '/region[@area="lead"]'
                                '/container[@module="archive-print-volume"]'
                                '/block/@href'):
                            volume_article_href = tree.xpath(
                                '/page/body/cluster[@area="feature"]'
                                '/region[@area="lead"]'
                                '/container[@module="archive-print-volume"]'
                                '/block/@href')[cnt]
            ...
            logger.error(
                "Exception _create_date_first_released" + uri + " " + str(e))
            return False
    ...
    def _get_property(self, connector, uri, key, namespace, logger):
        try:
            uri = unicode(uri.decode("utf8"))
            property = zeit.care.worker.get_property(
                connector[uri], key, namespace)
            return property
        except Exception as e:
...
#        attr = etree.Element('attribute', name=var,
#           ns="http://namespaces.zeit.de/CMS/workflow")
#        attr.text = val
        ...
        for uri in articles:
            ...
            if article:
                ...
                if tree:
                    # CASE - Product-Id
                    if mode == "productid":
                        productid = self._find_out_productid(
                            connector, uri, tree, logger)
                        if productid:
                            print productid
                            # http://namespaces.zeit.de/CMS/workflow urgent=yes
                            # make_checkable = self.make_checkable(
                            #   uri, connector, logger)
                            get_properties = self.get_properties(
                                uri, tree, ressorts, connector, logger)
                            # if not make_checkable or not get_properties:
                            if not get_properties:
                                logger.error(
                                    uri + ' Properties preparation error')
                            properties = {
                                ('product-id',
                                 'http://namespaces.zeit.de/CMS/workflow'):
                                productid}
                    ...
                    # CASE - Delete whitespace from authors begin
                        ...
                        author = self._delete_whitespace_author_begin(
                            tree, logger, uri)
                        if author:
                            # make_checkable = self.make_checkable(
                            #   uri, connector, logger)
                    ...
                    # CASE - Delete whitespaces from authors end
                        ...
                        author = self._delete_whitespace_author_end(
                            tree, logger, uri)
                        if author:
                            # make_checkable = self.make_checkable(
                            #   uri, connector, logger)
                    ...
                    # CASE - Date first released
                        ...
                        # self.wrong_date_attr_to_delete = False
                        datefirstrel = self._create_date_first_released(
                            connector, uri, tree, logger)
                        if datefirstrel:
                            # make_checkable = self.make_checkable(
                            #   uri, connector, logger)
                            ...
                            # if not self._detect_date_last_modified(tree):
                            if not self._get_property(
                                    connector, uri, 'date-last-modified',
                                    'http://namespaces.zeit.de/CMS/document',
                                    logger):
                                properties.update({
                                    ('date_last_modified',
                                    ...
                                    datefirstrel})
                            if not self._get_property(
                                    connector, uri, 'last-semantic-change',
                                    'http://namespaces.zeit.de/CMS/document',
                                    logger):
                                # if not self._detect_last_semantic_change(
                                #   tree):
                                properties.update({
                            ...
                            logger.error(
                                uri + ' Property error  date_first_released')

                    # CASE - Date last modified
                    elif mode == "datemodified":
                        datefirstrel = self._get_property(
                            connector, uri, 'date_first_released',
                            'http://namespaces.zeit.de/CMS/document', logger)
                        if datefirstrel:
                            # make_checkable = self.make_checkable(
                            #   uri, connector, logger)
                            get_properties = self.get_properties(
...
def main():
    ...
    if os.path.isfile(options.path + "/last_modified_semantic_change.txt"):
        message += (
            '\nlast_modified_sematic_change.txt '
            '- Will add missing attribute date_last_modified '
            'together with last_semantic_change')
        path_modified_semantic = (
            options.path + "/last_modified_semantic_change.txt")
    ...
    if not options.force:
        user_ok = raw_input(
            ...
            ' \n%s\n\nAre you sure? [y|n]: ' % (options.path, message))
    else:
        user_ok = "y"
    ...
    if user_ok == "y":
        ...
        if path_author_starts:
            ...
            authorstarts.run(
                ...
                'authorstarts', logger_error_authorstarts)
        ...
        if path_author_ends:
            ...
            authorends.run(
                ...
                'authorends', logger_error_authorends)
        ...
        if path_date_first:
            ...
            datefirst.run(
                ...
                'datefirst', logger_error_datefirst)
        ...
        if path_date_modified:
            ...
            datemodified.run(
                ...
                'datemodified', logger_error_datemodified)
        ...
        if path_semantic_change:
            ...
            semantic.run(
                ...
                connector, 'semanticchange', logger_error_semanticchange)
        ...
        if path_modified_semantic:
            ...
            modifiedsemantic.run(
                ...
                connector, 'modifiedsemantic', logger_error_modifiedsemantic)
        ...
        if path_productid:
            ...
            injectids.run(
                ...
                'productid', logger_error_productid)
            # injectids.list_copyrights(
            #     path_productid, options.logpath, options.string,
            #     logger_error_productid)
